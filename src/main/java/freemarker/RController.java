...
public class RController  {
	...
	static {
		audit = System.getenv("AUDIT_TRAIL").equals("true");
	}
    @Autowired
    ...
    public ResponseEntity<ReturnBean> postFile(@PathVariable("cols") Integer cols, @RequestBody InputBean input) {
        ...
        if (audit) beanRepository3.save(service.converter(input,rb,"result"+hexString+".pdf",date));
    ...
    public ResponseEntity<ReturnBean> postFile(@RequestBody InputBeanGeneral input) {
        ...
        rb.setSha1(Encryption.sha1(b64d.decode(result)));
        String hexString = Encryption.byteArrayToHexString(b64d.decode(rb.getSha1()));
        if (audit) beanRepository2.save(service.converter(input,rb,"result"+hexString+".pdf",date));
        return new ResponseEntity<ReturnBean>(rb, HttpStatus.OK);
    }
    /**
    ...
    public ResponseEntity<ReturnBean> postFile(@RequestBody InputBeanGeneral2 input) {
        ...
        if (System.getenv("OVERRIDE_WITH_HTML") != null && System.getenv("OVERRIDE_WITH_HTML").equals("true")) {
        	if (audit) beanRepository.save(service.converter(input,rb,"result.html",date));
        } else {
        	if (audit) beanRepository.save(service.converter(input,rb,"result"+hexString+".pdf",date));
        }
        return new ResponseEntity<ReturnBean>(rb, HttpStatus.OK);
    }
    //    /**
    ...
    public ResponseEntity<ReturnBean> toPDF(@RequestBody InputHTMLString input) {
        ...
        rb.setSha1(Encryption.sha1(b64d.decode(result)));
        String hexString = Encryption.byteArrayToHexString(b64d.decode(rb.getSha1()));
        if (audit) beanRepository4.save(service.converter(input,rb,"result"+hexString+".pdf",date));
    ...
    public synchronized ResponseEntity<Boolean> saveTemplate(@RequestBody TemplateBean input) {
        Date date = new Date();
        if (audit) beanRepository6.deleteByName(input.getName());
        if (audit) beanRepository6.save(service.converter(input, date));
